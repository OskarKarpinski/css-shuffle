...
export function isDomElement(node: t.Node, scope: any): boolean {
    ...
    if (t.isIdentifier(node)) {
        ...
        if (bindingPath.node.type === 'Identifier' && bindingPath.parentPath) {
            ...
            if (
                t.isArrowFunctionExpression(parent) || t.isFunctionExpression(parent)
            ) {
                ...
                if (
                    ...
                    t.isIdentifier((callPath.node.callee as t.MemberExpression).property) &&
                    ['forEach', 'map', 'filter', 'find'].includes(
                        ((callPath.node.callee as t.MemberExpression).property as t.Identifier).name
                    )
                ) {
